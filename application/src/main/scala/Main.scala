...
object Main{
  ...
  def databaseFill(): Unit = {
    ...
    for(i <- 1 to 25){
      Await.result(staffRepository.create(Staff(Some(i), s"Staff #$i", i % 5, 20 + i)), Duration.Inf)
    }

    for(i <- 1 to 100){
      Await.result(filmRepository.create(
        Film(Some(i), s"Film #$i", (i * 10).seconds, i % 5 + 2, i % 5), List(i % 10 + 1, i % 5 + 1), List(i % 7 + 1, i % 10 + 1), List(i % 5 + 1)
      ), Duration.Inf)
    }
  }
}
